...
// Bank Data Refleksi Guru yang sering terjadi di kelas
const REFLECTION_SUGGESTIONS = [
  { category: 'Positif', label: 'Murid sangat antusias dan aktif berdiskusi kelompok', color: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
...
// Bank Data Rencana Tindak Lanjut (RTL) yang relevan dengan Kurikulum Merdeka
...
const PostObservation: React.FC<Props> = ({ observations, onSave }) => {
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm">Tahap refleksi dan pemberian umpan balik produktif alur TIRTA.</p>
          </div>
        ...
        {currentList.length === 0 ? (
          <div className="py-24 bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center text-center px-6">
            <div className={`p-6 rounded-full mb-4 ${viewMode === 'pending' ? 'bg-blue-50 text-blue-400' : 'bg-emerald-50 text-emerald-400'}`}>
            ...
            <p className="text-slate-500 max-w-sm mt-2 text-sm">
              {viewMode === 'pending' 
                ...
                : 'Setelah Anda menyelesaikan sesi coaching, datanya akan tersimpan di sini.'}
            </p>
          </div>
        ) : (
              ...
              return (
                <div key={obs.teacherId} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900">{teacher?.name || 'Guru Tidak Teridentifikasi'}</p>
                      <p className="text-xs text-slate-500">{teacher?.subject} • {new Date(obs.date).toLocaleDateString('id-ID')}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => handleSelect(obs)}
                    className={`px-5 py-3 rounded-2xl text-sm font-bold transition-all flex items-center space-x-2 ${
                      viewMode === 'pending' 
                      ? 'bg-blue-600 text-white hover:bg-blue-700' 
                      : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                    }`}
                  >
                    <span>{viewMode === 'pending' ? 'Mulai Coaching' : 'Lihat Detail'}</span>
                  </button>
                </div>
              );
            })}
  ...
  const currentTeacher = TEACHERS.find(t => t.id === selectedObs.teacherId);
    ...
    <div className="space-y-8 animate-in slide-in-from-right-8 duration-500 pb-20">
      <div className="flex items-center justify-between">
        <button onClick={() => setSelectedObs(null)} className="text-slate-500 hover:text-slate-900 font-bold flex items-center text-sm group">
          <RefreshCcw size={16} className="mr-2 group-hover:rotate-180 transition-transform duration-500" /> Kembali ke Daftar
        </button>
        <div className="text-right">
          <h2 className="text-xl font-bold text-slate-900">{currentTeacher?.name}</h2>
          <p className="text-xs text-slate-500">{currentTeacher?.subject} • {currentTeacher?.phase}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-8">
            ...
            {/* Section 1: Refleksi Guru dengan Pilihan Cepat */}
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 space-y-4">
                <div className="flex items-center text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">
                  <Lightbulb size={12} className="mr-2 text-amber-500" /> Pilih Kejadian yang Sesuai (Klik untuk Menambah):
                </div>
                ...
                <textarea 
                  ...
                  placeholder="Atau tuliskan refleksi guru secara manual di sini..."
                  className="w-full bg-white border border-slate-200 p-5 rounded-2xl h-32 text-sm outline-none focus:ring-2 focus:ring-blue-500 transition-all shadow-inner"
                />
              </div>
            </div>

            {/* Section 2: Umpan Balik AI */}
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-sm font-bold text-slate-700 flex items-center">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-600 mr-2"></span>
                  2. Umpan Balik Kepala Sekolah (Alur TIRTA)
                </label>
                <button 
                  onClick={handleGenerateAI}
                  disabled={isGenerating}
                  className="flex items-center space-x-2 text-xs font-bold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-full hover:bg-indigo-100 transition-all border border-indigo-100"
                >
                  <Sparkles size={14} />
                  <span>{isGenerating ? 'AI sedang menganalisis...' : 'Dapatkan Saran AI'}</span>
                </button>
              </div>
              <textarea 
                value={feedback}
                onChange={(e) => setFeedback(e.target.value)}
                placeholder="Tujuan, Identifikasi, Rencana Aksi, Tanggung Jawab..."
            ...
            {/* Section 3: RTL dengan Pilihan Cepat */}
            <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 space-y-4">
                <div className="flex items-center text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">
                  <MousePointer2 size={12} className="mr-2" /> Pilih RTL Relevan (Klik untuk Menambah):
                </div>
                <div className="flex flex-wrap gap-2">
                ...
                <div className="pt-2">
                  <textarea 
                    ...
                    placeholder="Tuliskan komitmen guru untuk perbaikan selanjutnya..."
                    className="w-full bg-white border border-slate-200 p-5 rounded-2xl h-32 text-sm outline-none focus:ring-2 focus:ring-emerald-500 transition-all shadow-inner"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-start">
                <div className="bg-blue-500/20 text-blue-400 p-2 rounded-lg mr-4 font-bold">T</div>
                <div>
                  <p className="font-bold text-sm">Tujuan</p>
                  <p className="text-[10px] text-slate-400">Sepakati tujuan percakapan</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="bg-blue-500/20 text-blue-400 p-2 rounded-lg mr-4 font-bold">I</div>
                <div>
                  <p className="font-bold text-sm">Identifikasi</p>
                  <p className="text-[10px] text-slate-400">Gali fakta-fakta yang terjadi</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="bg-blue-500/20 text-blue-400 p-2 rounded-lg mr-4 font-bold">R</div>
                <div>
                  <p className="font-bold text-sm">Rencana Aksi</p>
                  <p className="text-[10px] text-slate-400">Kembangkan ide solusi dari guru</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="bg-blue-500/20 text-blue-400 p-2 rounded-lg mr-4 font-bold">TA</div>
                <div>
                  <p className="font-bold text-sm">Tanggung Jawab</p>
                  <p className="text-[10px] text-slate-400">Komitmen langkah selanjutnya</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-[2rem]">
             <div className="flex items-center space-x-3 text-emerald-700 mb-3">
               <ListChecks size={20} />
               <p className="text-sm font-bold">Tips RTL Efektif</p>
             </div>
             <ul className="space-y-3">
               {[
                 'Harus spesifik dan terukur',
                 'Memiliki target waktu yang jelas',
                 'Disesuaikan dengan minat guru',
                 'Mendukung peningkatan kualitas murid'
               ].map((tip, i) => (
                 <li key={i} className="flex items-start text-[10px] text-emerald-800">
                   <div className="w-1 h-1 rounded-full bg-emerald-400 mt-1.5 mr-2"></div>
                   {tip}
                 </li>
               ))}
             </ul>
